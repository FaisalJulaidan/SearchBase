...
def sendDemoRequest(email) -> Callback:
    try:
        send_email(tsbEmail, 'Demo Request', '/emails/verification.html', email=email)
...
def contactUsIndex(name, email, message) -> Callback:
    try:
        send_email(tsbEmail, 'TheSearchBase Contact Us', '/emails/verification.html', name=name, email=email,
                   message=message)
